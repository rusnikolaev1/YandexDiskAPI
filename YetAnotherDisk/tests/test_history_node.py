...
def test_history_node_file_success(client):
    ...
    assert response.data == {"items": [item_1_ver_2, item_1_ver_1]}


@pytest.mark.django_db
def test_history_node_folder_success(client):
    """
    ...
    assert response.data == {"items": [item_1_ver_3, item_1_ver_2, item_1_ver_1]}
...
def test_history_node_error_deleted_items(client):
    ...
    response = client.delete(URL_DELETE + f"/{id}?date=" + dt_2)

    # GET запрос по адресу /node/{id}/history
    dt_3 = (
        convert_str_to_datetime(DATE_1) + datetime.timedelta(microseconds=2)
    ).strftime("%Y-%m-%dT%H:%M:%S.%f") + "Z"
    response = client.get(
        URL_HISTORY_NODE
        + f"/{id}/history"
...
def test_history_node_without_date_return_all_history(client):
    ...
    dt_2 = (convert_str_to_datetime(DATE_1) + datetime.timedelta(hours=1)).strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    ) + "Z"
    ...
    dt_3 = (convert_str_to_datetime(dt_2) + datetime.timedelta(hours=1)).strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    ) + "Z"
    ...
    response = client.get(URL_HISTORY_NODE + f"/{id}/history")
