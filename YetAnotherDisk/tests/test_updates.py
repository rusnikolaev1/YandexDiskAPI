...
def test_updates_file_success(client):
    ...
    item_3['date'] = DATE_1
    item_3['children'] = None
    assert response.data == [item_1, item_3]
...
def test_updates_file_success_only_24_hours(client):
    ...
    Item.objects.create(
        ...
        size=item_3['size'],
        url=item_3['url'],
        date=convert_str_to_datetime(DATE_1) + datetime.timedelta(microseconds=1,)
    )
    ...
    Item.objects.create(
        ...
        size=item_4['size'],
        url=item_4['url'],
        date=convert_str_to_datetime(DATE_1) - datetime.timedelta(days=-1, microseconds=-1)
    )
    ...
    item_1['date'] = DATE_1
    item_1['children'] = None
    item_2['date'] = (convert_str_to_datetime(DATE_1) + datetime.timedelta(hours=-12)).strftime('%Y-%m-%dT%H:%M:%S') + 'Z'
    item_2['children'] = None
    assert response.data == [item_1, item_2]
